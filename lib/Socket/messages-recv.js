...
const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, maxMsgRetryCount, getMessage, shouldIgnoreJid, shouldIgnoreParticipant, ignoreOfflineMessages, resendReceipt } = config;
    ...
    const handleReceipt = async (node) => {
        ...
        if ((0, WABinary_1.isJidGroup)(attrs.from) && shouldIgnoreParticipant(attrs.participant)) {
            ...
            processingMutex.mutex(async () => {
                ...
                if (attrs.type === 'retry' && key.fromMe && resendReceipt) {
                    if (willSendMessageAgain(ids[0], key.participant)) {
                        const retryUser = msgRetryCache.get(key.participant) || 0;
                        if (retryUser < 2) {
    ...
    const handleNotification = async (node) => {
        ...
        if (!((_a = node.attrs) === null || _a === void 0 ? void 0 : _a.from)) {
        ...
        if ((0, WABinary_1.isJidGroup)(remoteJid) && shouldIgnoreParticipant(node.attrs.participant)) {
            logger.debug({ remoteJid, id: node.attrs.id }, 'ignored notification');
            await sendMessageAck(node);
            return;
        }
    ...
    const handleMessage = async (node) => {
        var _a, _b, _c, _d;
        if (!((_a = node.attrs) === null || _a === void 0 ? void 0 : _a.from)) {
            logger.debug({ from: ((_b = node.attrs) === null || _b === void 0 ? void 0 : _b.from) || null, id: node.attrs.id }, 'ignored null jid');
            await sendMessageAck(node);
            return;
        }
        ...
        if ((0, WABinary_1.isJidGroup)(node.attrs.from) && shouldIgnoreParticipant(node.attrs.participant)) {
            logger.debug({ key: node.attrs.key }, 'ignored participant message');
            await sendMessageAck(node);
            return;
        }
        if (shouldIgnoreJid(node.attrs.from) && node.attrs.from !== '@s.whatsapp.net' && !(0, WABinary_1.areJidsSameUser)(node.attrs.from, authState.creds.me.id)) {
            logger.debug({ key: node.attrs.key }, 'ignored message');
            await sendMessageAck(node);
            return;
        }
        const { fullMessage: msg, category, author, decrypt } = (0, Utils_1.decryptMessageNode)(node, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, logger);
        if (((_d = (_c = msg.message) === null || _c === void 0 ? void 0 : _c.protocolMessage) === null || _d === void 0 ? void 0 : _d.type) === WAProto_1.proto.Message.ProtocolMessage.Type.SHARE_PHONE_NUMBER) {
